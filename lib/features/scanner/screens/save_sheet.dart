...
class _SaveSheetState extends State<SaveSheet> {
  ...
  Future<void> _save() async {
    ...
    if (doc != null) {
      ...
      // Best-effort: if upload fails we still have the local copy
      _uploadToApi(doc, name);
      ...
      messenger.showSnackBar(SnackBar(
        content: Row(children: [
          ...
          const SizedBox(width: 8),
          Expanded(
              child: Text('Saved "${doc.name}" to Digital Locker',
  ...
  /// Upload the locally-saved scan to the backend Documents API.
  /// Runs in the background — failures are silently swallowed since the user
  /// already has a local copy in the scanner vault.
  Future<void> _uploadToApi(ScanDocument doc, String name) async {
    try {
      // Use the file path stored in the ScanDocument (PDF or JPEG)
      final filePath = doc.savedPdfPath ?? (doc.pages.isNotEmpty ? doc.pages.first.processedPath : null);
      ...
      final formData = FormData.fromMap({
        'title': name,
        'type': _cat == DocCategory.other ? 'other' : _cat.name,
        'file': await MultipartFile.fromFile(
          filePath,
          filename: filePath.split('/').last,
        ),
        if (_expiry != null) 'expiry_date': _expiry!.toIso8601String().substring(0, 10),
      });

      await provider.uploadDocument(formData);
    } catch (_) {
      // Silent — local copy is authoritative
    }
  }
}
