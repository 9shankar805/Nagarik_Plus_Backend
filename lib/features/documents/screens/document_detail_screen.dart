...
class _DocumentDetailScreenState extends State<DocumentDetailScreen>
  ...
  /// Refresh from the real API if the document has a numeric backend id.
  Future<void> _refreshDoc() async {
    final docId = int.tryParse(widget.document.id);
    if (docId == null) return; // local-only document (scanner vault), skip API call
    try {
      final updated = await context.read<DocumentsProvider>().getDocById(docId);
      if (updated != null && mounted) {
        setState(() => _doc = updated);
      }
    } catch (_) {
      // Not critical — continue with whatever was passed in
    }
  }
  ...
  Future<void> _processAndSaveFile(File sourceFile, {required bool isBack}) async {
    try {
      ...
      if (mounted) setState(() => _doc = updatedDoc);

      // Upload to backend API
      final docProvider = context.read<DocumentsProvider>();
      final docId = int.tryParse(_doc.id);

      if (docId != null) {
        // Document already exists on server — update file via upload endpoint
        try {
          ...
          await docProvider.uploadDocument(formData);
        } catch (_) {
          // Upload failed — local copy still saved, not critical
        }
      } else {
        // Brand-new document — create on server
        try {
          final formData = FormData.fromMap({
            'title': _doc.title,
            'type': _doc.type,
          ...
          // Upload failed — local copy still saved
        }
      }
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Row(children: [
            ...
            Expanded(
              child: Text(
                '${_doc.title} (${isBack ? "Back Side" : "Front Side"}) saved & uploaded to secure locker!',
            ...
            onPressed: () async {
              ...
              try {
                ...
                if (docId != null) {
                  ...
                  final fileName = doc.subtitle.isNotEmpty
                      ? '${doc.type}_${docId}.pdf'
                      : 'document_${docId}.pdf';
                  final savedPath = await docProvider.downloadDoc(docId, fileName: fileName);
                  if (mounted) {
                    messenger.hideCurrentSnackBar();
                    messenger.showSnackBar(
                      SnackBar(
                        content: Text('Saved to: ${savedPath.split('/').last}'),
                  ...
                  messenger.showSnackBar(
                    const SnackBar(content: Text('This document has no server file to download.')),
                  );
                }
              } catch (e) {
                if (mounted) {
