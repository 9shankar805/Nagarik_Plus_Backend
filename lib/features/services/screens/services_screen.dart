...
class _ServicesScreenState extends State<ServicesScreen> with SingleTickerProviderStateMixin {
  ...
  void didChangeDependencies() {
    ...
    // We can't initialize TabController in initState if we need to read ModalRoute arguments,
    // so we do it in didChangeDependencies the first time it is called.
    if (!_isTabInitialized) {
      final args = ModalRoute.of(context)?.settings.arguments as Map<String, dynamic>?;
      final int initialIndex = args?['initialTab'] ?? 0;
  ...
  final List<_ServiceData> _socialServices = [
    _ServiceData(
      title: 'Citizenship Information',
      subtitle: 'Apply for Nepali citizenship',
      icon: Icons.settings_rounded,
      color: const Color(0xFF1565C0),
      eligibility: 'All Nepali citizens',
      documents: ['Birth Certificate', 'Parent\'s Citizenship'],
      steps: ['Visit Ward Office', 'Verify documents', 'Submit application to CDO'],
      fee: 'Varies',
      processingTime: '1-3 days',
      faqs: ['Can I apply online?'],
    ),
    _ServiceData(
      ...
      eligibility: 'Employees registered with EPF',
      documents: ['EPF Number', 'Identity Card'],
      steps: ['Login to EPF portal', 'View statements'],
      fee: 'Free',
      processingTime: 'Instant',
      faqs: ['How to withdraw?'],
    ),
    _ServiceData(
      ...
      color: const Color(0xFF1565C0),
      eligibility: 'Formal and informal sector workers',
      documents: ['SSF Number'],
      steps: ['Login to SSF portal', 'View contributions'],
      fee: 'Free',
      processingTime: 'Instant',
  ...
  List<_ServiceData> get _filteredSocialServices {
    if (_searchQuery.isEmpty) return _socialServices;
            ...
            SliverAppBar(
              ...
              flexibleSpace: FlexibleSpaceBar(
                background: Stack(
                  ...
                  children: [
                    Image.asset(
                      'assets/Banner/temple_24.webp', // Fallback to string if AppAssets.templeBanner is missing
                      fit: BoxFit.cover,
                    ),
                    Container(
                      color: AppColors.primary.withValues(alpha: 0.8), // Darken the image for text readability
                    ),
                  ],
                ),
              ),
              bottom: TabBar(
                ...
                tabs: [
                  Tab(text: context.l10n.citizenServices),
                  Tab(text: isNepali ? 'सामाजिक सेवा' : 'Social Services'),
                ],
              ),
            ),
          ];
      ...
      Column(
        children: [
          ...
          Expanded(
            child: ListView.builder(
              padding: const EdgeInsets.all(16),
              itemCount: _filteredSocialServices.length,
              itemBuilder: (context, index) {
                final service = _filteredSocialServices[index];
                return _ServiceCard(
                  service: service,
                  onTap: () => Navigator.of(context).push(
                    MaterialPageRoute(
