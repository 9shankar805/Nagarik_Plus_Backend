...
class _NewsScreenState extends State<NewsScreen>
  ...
  // Fallback demo posts wrapped in a ValueNotifier so toggling like/bookmark
  // on a single card only rebuilds that card — not the entire NewsScreen.
  late final ValueNotifier<List<NewsFeedPost>> _postsNotifier =
      ValueNotifier(_buildFallbackPosts());

  List<NewsFeedPost> get _posts => _postsNotifier.value;

  List<NewsFeedPost> _buildFallbackPosts() => [
    NewsFeedPost(
      id: 'post_1',
            ...
            IconButton(
              ...
              onPressed: () {
                Navigator.of(context).push(
                  MaterialPageRoute(
                    builder: (_) => CreateShortScreen(isNepali: isNepali),
  ...
  Widget _buildNewsFeedTab(bool isNepali, NewsProvider provider) {
    ...
    return RefreshIndicator(
      ...
      // ValueListenableBuilder wraps only the list — if _postsNotifier changes
      // (like/bookmark toggle) only this subtree rebuilds, not the AppBar/TabBar.
      child: ValueListenableBuilder<List<NewsFeedPost>>(
        valueListenable: _postsNotifier,
        builder: (context, fallbackArticles, _) {
          return ListView.builder(
            padding: const EdgeInsets.only(bottom: 24),
            itemCount: (displayArticles?.length ?? fallbackArticles.length) + 3,
            itemBuilder: (context, index) {
              if (index == 0) {
                return _buildCreatePostArea(isNepali);
              } else if (index == 1) {
          ...
          if (displayArticles != null) {
            ...
            // newsArticleToPost is also used for comments — cache it once per item build
            ...
            return _FacebookPostCard(
              ...
              isNepali: isNepali,
              onLikeToggle: () {
                // Mutate a copy of the list and notify — only cards backed by
                // ValueListenableBuilder rebuild, not the entire NewsScreen.
                final updated = List<NewsFeedPost>.from(_postsNotifier.value);
                final idx = updated.indexWhere((p) => p.id == post.id);
                if (idx != -1) {
                  final p = updated[idx];
                  updated[idx] = NewsFeedPost(
                    id: p.id,
                    sourceEn: p.sourceEn, sourceNe: p.sourceNe,
                    sourceUrl: p.sourceUrl,
                    timeEn: p.timeEn, timeNe: p.timeNe,
                    categoryEn: p.categoryEn, categoryNe: p.categoryNe,
                    titleEn: p.titleEn, titleNe: p.titleNe,
                    contentEn: p.contentEn, contentNe: p.contentNe,
  ...
  // ── Facebook Style "Create Post" Area ──────────────────────────────────────
  Widget _buildCreatePostArea(bool isNepali) {
    return Container(
      color: Colors.white,
        ...
        children: [
          Row(
            children: [
              ...
              Expanded(
                child: InkWell(
                  onTap: () {
                    Navigator.of(context).push(
                      MaterialPageRoute(
                        builder: (_) => CreatePostScreen(isNepali: isNepali),
                        fullscreenDialog: true,
                      ),
                    );
                  },
                  ...
                  child: Container(
                    ...
                    child: Text(
                      isNepali ? 'समाचार वा विचार राख्नुहोस्...' : "Share news, issues, or thoughts...",
              ...
              const SizedBox(width: 12),
              const Icon(Icons.photo_library_rounded, color: Colors.green, size: 28),
            ],
          ),
          ...
          Row(
            ...
            children: [
              _buildPostAction(context, isNepali, Icons.videocam_rounded, Colors.red, isNepali ? 'लाइभ भिडियो' : 'Live video'),
              _buildPostAction(context, isNepali, Icons.photo_library_rounded, Colors.green, isNepali ? 'फोटो/भिडियो' : 'Photo/video'),
              _buildPostAction(context, isNepali, Icons.location_on_rounded, Colors.orange, isNepali ? 'स्थान' : 'Location'),
            ],
          ),
        ],
      ),
  ...
  Widget _buildPostAction(BuildContext context, bool isNepali, IconData icon, Color color, String label) {
    return InkWell(
      onTap: () {
        Navigator.of(context).push(
          MaterialPageRoute(
...
class _NewsArticleCard extends StatelessWidget {
  ...
  // Conversion runs on demand in build.
  NewsFeedPost get _post => newsArticleToPost(article);
                    ...
                    ScaffoldMessenger.of(context).showSnackBar(
                      SnackBar(
                        content: Text(isSaved
                            ? (widget.isNepali ? 'सर्ट सुरक्षित गरियो!' : 'Short saved!')
                            : (widget.isNepali ? 'सर्ट सुरक्षित हटाइयो!' : 'Short removed from saved!')),
                        duration: const Duration(seconds: 1),
                      ),
                    );
                  },
