...
class _MainScreenState extends State<MainScreen> {
  ...
  Widget? _buildBottomNav() {
    // News screen manages its own full-screen UI — no nav bar
    if (_currentIndex == 1) return null;

    // All other tabs use the shared floating pill nav
    return _NavBar(
  ...
  Widget build(BuildContext context) {
    // Status bar style changes depending on which screen is active
    final onLearning = _currentIndex == 2;
    SystemChrome.setSystemUIOverlayStyle(SystemUiOverlayStyle(
      statusBarColor: Colors.transparent,
      statusBarIconBrightness:
          onLearning ? Brightness.light : Brightness.dark,
      systemNavigationBarColor:
          onLearning ? const Color(0xFF24243E) : const Color(0xFFF2F4F8),
