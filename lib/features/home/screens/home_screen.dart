...
import '../../documents/screens/document_category_screen.dart';
import '../../news/providers/news_provider.dart';
import '../../office_locator/screens/office_locator_screen.dart';
import '../../scanner/repositories/scanner_repository.dart';
import '../../scanner/models/scan_document.dart';
import '../../scanner/screens/pdf_viewer_screen.dart';
import 'dart:io';
...
class _HomeScreenState extends State<HomeScreen> {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: CustomScrollView(
          slivers: [
            // Top bar only rebuilds when NotificationProvider changes (scoped inside)
            SliverToBoxAdapter(child: _buildTopBar(context)),
            // Banner is now its own StatefulWidget — page swipe setState is isolated
            const SliverToBoxAdapter(child: _HomeBannerSection()),
            SliverToBoxAdapter(child: _buildNagarikAdvisorsSection(context)),
            // Documents section is scoped to DocumentsProvider only
            SliverToBoxAdapter(child: _buildDocuments(context)),
            // Social services scoped to HomeProvider only
            SliverToBoxAdapter(child: _buildSocialServices(context)),
            // Vital events scoped to HomeProvider only
            SliverToBoxAdapter(child: _buildVitalEvents(context)),
            SliverToBoxAdapter(child: _buildQuickServices(context)),
            const SliverToBoxAdapter(child: _RecentScansWidget()),
  ...
  // Uses Consumer so only this section rebuilds when DocumentsProvider notifies,
  // not the entire HomeScreen.
  Widget _buildDocuments(BuildContext context) {
    return Consumer<DocumentsProvider>(
      builder: (context, documentsProvider, _) {
  ...
  // Scoped to HomeProvider only — DocumentsProvider changes won't trigger a rebuild.
  ...
  void _handleVitalEventTap(BuildContext context, String? type, String label) {
    String resolvedType = type ?? "";
    if (resolvedType.isEmpty) {
      final titleLower = label.toLowerCase();
      if (titleLower.contains("birth") || titleLower.contains("जन्म")) { resolvedType = "birth_certificate"; }
      else if (titleLower.contains("marriage") || titleLower.contains("विवाह")) { resolvedType = "marriage_certificate"; }
      else if (titleLower.contains("death") || titleLower.contains("मृत्यु")) { resolvedType = "death_certificate"; }
      else if (titleLower.contains("migration") || titleLower.contains("बसाइँ")) { resolvedType = "migration_certificate"; }
    }

    if (resolvedType.isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('$label not found in your profile.'),
    ...
    return Consumer<HomeProvider>(
      builder: (context, homeProvider, _) {
        ...
        final fallbackEvents = [
          _VitalItem(context.l10n.birthCertificate,     AppAssets.birthCert,     const Color(0xFFE8F5E9), 'birth_certificate'),
          _VitalItem(context.l10n.marriageCertificate,  AppAssets.marriageCert,  const Color(0xFFFFE4EC), 'marriage_certificate'),
          _VitalItem(context.l10n.deathCertificate,     AppAssets.deathCert,     const Color(0xFFF3E5F5), 'death_certificate'),
          _VitalItem(context.l10n.migrationCertificate, AppAssets.migrationCert, const Color(0xFFFFF8E1), 'migration_certificate'),
        ];
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, i) {
                if (displayEvents != null) {
                  ...
                  return GestureDetector(
                    onTap: () => _handleVitalEventTap(context, e.type, e.title ?? ''),
                    child: Container(
                      width: 88,
                  ...
                  return GestureDetector(
                    onTap: () => _handleVitalEventTap(context, e.documentType, e.label),
                    child: Container(
                      width: 88,
                      margin: const EdgeInsets.only(right: 10),
                      child: Column(
...
// ── Banner carousel — isolated StatefulWidget ─────────────────────────────────
// Owning PageController and _bannerIndex here means banner page-swipe setState
// only rebuilds this subtree, not the entire HomeScreen.
...
class _HomeBannerSectionState extends State<_HomeBannerSection> {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<HomeProvider>(
      // Rebuilds only this widget when HomeProvider loads banner data.
      builder: (context, homeProvider, _) {
        final validBanners =
            homeProvider.banners.where((b) => b.imageUrl != null).toList();
        final hasApiBanners = validBanners.isNotEmpty;
        ...
        final fallbackBanners = [
          _BannerData(
            ...
            subtitleNp: isNepali
                ...
                : 'Anywhere, anytime – secure, fast & reliable.',
            chips: [
              _ChipData(Icons.shield_rounded, isNepali ? 'सुरक्षित' : 'Secure',
                  isNepali ? 'तपाईंको उज्ज्वल सुरक्षा' : 'Your safety'),
              _ChipData(Icons.bolt_rounded, isNepali ? 'छिटो' : 'Fast',
                  isNepali ? 'केही सेकेन्डमै अद्यावधिक' : 'Instant update'),
              _ChipData(Icons.check_circle_rounded, isNepali ? 'सजिलो' : 'Easy',
                  isNepali ? 'सजिलो डिजिटल सेवा' : 'Easy service'),
            ],
          ...
          _BannerData(
            ...
            subtitleNp: isNepali
                ? 'आफ्ना सबै कागजात एकै ठाउँमा राख्नुस्।'
                : 'Keep all your document cards in one place.',
            chips: [
              _ChipData(Icons.lock_rounded, isNepali ? 'एन्क्रिप्टेड' : 'Encrypted',
                  isNepali ? 'AES-256 सुरक्षा' : 'AES-256 Security'),
              _ChipData(Icons.cloud_done_rounded, isNepali ? 'क्लाउड' : 'Cloud',
                  isNepali ? 'जहाँबाट पनि पहुँच' : 'Access anywhere'),
              _ChipData(Icons.verified_rounded, isNepali ? 'प्रमाणित' : 'Verified',
                  isNepali ? 'सरकारी मान्यताप्राप्त' : 'Govt verified'),
            ],
          ...
          _BannerData(
            ...
            chips: [
              _ChipData(Icons.book_rounded, isNepali ? 'पासपोर्ट' : 'Passport',
                  isNepali ? 'अनलाइन आवेदन' : 'Online apply'),
              _ChipData(Icons.receipt_rounded, isNepali ? 'PAN' : 'PAN',
                  isNepali ? 'दर्ता र नवीकरण' : 'Reg & renewal'),
              _ChipData(Icons.badge_rounded, isNepali ? 'NID' : 'NID',
                  isNepali ? 'नागरिकता सेवा' : 'NID service'),
            ],
        ...
        final count = hasApiBanners ? validBanners.length : fallbackBanners.length;
            ...
            children: [
              SizedBox(
                ...
                child: PageView.builder(
                  ...
                  itemBuilder: (_, index) => hasApiBanners
                      ? _ApiBannerCard(banner: validBanners[index])
                      : _BannerCard(data: fallbackBanners[index]),
                ),
              ),
              const SizedBox(height: 10),
              Row(
...
class _VitalItem {
  ...
  const _VitalItem(this.label, this.imagePath, this.bgColor, this.documentType);
}
...
class _RecentScansWidgetState extends State<_RecentScansWidget> {
  // Each entry: the ScanDocument + a pre-checked bool for thumbnail existence.
  // File.exists() is async and called in _load(), never in build().
  List<({ScanDocument doc, bool thumbExists})> _items = [];
  bool _loading = true;
  ...
  Future<void> _load() async {
    final docs = (await ScannerRepository.instance.loadAll()).take(3).toList();

    // Pre-check thumbnail existence asynchronously — safe to do here.
    final items = await Future.wait(docs.map((doc) async {
      final thumb = doc.pages.isNotEmpty ? doc.pages.first.processedPath : null;
      final exists = thumb != null && await File(thumb).exists();
      return (doc: doc, thumbExists: exists);
    }));
              ...
              children: [
                const Text('Recent Scans',
                    style: TextStyle(
                        ...
                        color: Color(0xFF212121))),
                GestureDetector(
                  onTap: () => Navigator.of(context).pushNamed(AppRoutes.scannerVault),
                  child: Text(context.l10n.viewAll,
                      style: const TextStyle(
                          fontSize: 12,
                          fontWeight: FontWeight.w600,
            ...
            return GestureDetector(
              ...
              child: Container(
                ...
                decoration: const BoxDecoration(
                  ...
                  boxShadow: [AppColors.thinShadow],
                ),
                child: Row(
                  children: [
                    Container(
                      ...
                      child: ClipRRect(
                        ...
                        // Use pre-checked bool — no sync I/O in build
                        child: item.thumbExists
                    ...
                    Text(DateFormat('MMM dd').format(doc.createdAt),
                        style: const TextStyle(
                            color: AppColors.textLight, fontSize: 11, fontWeight: FontWeight.w600)),
                  ],
                ),
              ),
            );
