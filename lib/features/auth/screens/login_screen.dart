...
import '../../scanner/services/biometric_service.dart';
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _checkBiometricPreference() async {
    ...
    final enabled = await bioService.isLoginBiometricEnabled();
    final canUse = await bioService.isBiometricAvailable();
    if (mounted) {
      setState(() {
        _showBiometricButton = enabled && canUse;
      });
    }
  }
  ...
  Future<void> _checkBiometricPrompt() async {
    final bioService = BiometricService.instance;
    final canUse = await bioService.isBiometricAvailable();
    final hasPrompted = await bioService.hasPromptedForBiometric();
    
    if (canUse && !hasPrompted) {
      if (!mounted) return;
      final enable = await showDialog<bool>(
        ...
        builder: (context) => AlertDialog(
          ...
          title: const Row(children: [
            ...
            SizedBox(width: 10),
            Text('Enable Biometric Login?', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700)),
          ]),
          content: const Text('Would you like to use your fingerprint or Face ID to quickly log in next time?'),
          actions: [
            TextButton(
      ...
      final choice = enable ?? false;
      await bioService.setLoginBiometricEnabled(choice);
      await bioService.setPromptedForBiometric();
      
      if (choice && mounted) {
        try {
          await Provider.of<ProfileProvider>(context, listen: false).updateProfile({'biometric_enabled': true});
        } catch (_) {}
      }
    }
  }
        ...
        if (didAuthenticate && mounted) {
          ...
          if (authProvider.status == AuthStatus.authenticated) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('No saved account found. Please login with your email and password first.'),
