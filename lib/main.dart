...
import 'features/scanner/screens/vault_screen.dart';
  ...
  runApp(
    MultiProvider(
      providers: [
        ...
        // Core providers — accessed on first screen, eager is fine
        ...
        // Feature providers — lazy: true ensures create() only runs when first read.
        // In provider v6 this is already the default, but we make it explicit
        // to guard against future regressions (e.g. someone adding an init call).
        ChangeNotifierProvider<AdvisorsProvider>(
            lazy: true, create: (_) => AdvisorsProvider()),
        ChangeNotifierProvider<AdvisorPaymentProvider>(
            lazy: true, create: (_) => AdvisorPaymentProvider()),
        ChangeNotifierProvider<ServicesProvider>(
            lazy: true, create: (_) => ServicesProvider()),
        ChangeNotifierProvider<EmergencyProvider>(
            lazy: true, create: (_) => EmergencyProvider()),
        ChangeNotifierProvider<RemindersProvider>(
            lazy: true, create: (_) => RemindersProvider()),
        ChangeNotifierProvider<AiProvider>(
            lazy: true, create: (_) => AiProvider()),
        ChangeNotifierProvider<OfficeProvider>(
            lazy: true, create: (_) => OfficeProvider()),
        ChangeNotifierProvider<LearningProvider>(
            lazy: true, create: (_) => LearningProvider()),
        ChangeNotifierProvider<SyncProvider>(
            lazy: true, create: (_) => SyncProvider()),
      ],
      ...
      builder: (context, localeProvider, _) {
        return MaterialApp(
          ...
          routes: {
            ...
            // Scanner vault
            AppRoutes.scannerVault: (_) => const VaultScreen(),
            // Learning center
            AppRoutes.learning: (_) => const LearningScreen(),
          },
        );
      },
    );
