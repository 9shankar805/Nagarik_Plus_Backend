...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-900 mt-2">Add Tutorial Quiz Question</h1>
</div>

<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 max-w-3xl">
    <form action="{{ route('admin.quiz.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
        @csrf
        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Question (English) <span class="text-red-500">*</span></label>
            <textarea name="question" required rows="2" placeholder="e.g. What does a triangular red border traffic sign mean?"
        ...
        <div>
            ...
            <textarea name="question_np" rows="2" placeholder="उदा. रातो घेरा भएको त्रिभुजाकार ट्राफिक चिन्हले के जनाउँछ?"
                      class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm"></textarea>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <select name="category" required class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                    <option value="driving_license">Driving License Preparation</option>
                    <option value="traffic_rules">Traffic Rules & Signals</option>
                    <option value="lok_sewa">Lok Sewa General Knowledge</option>
                    <option value="citizen_duties">Citizen Rights & Duties</option>
                </select>
            </div>
            <div>
        ...
        <div class="grid grid-cols-2 gap-4 p-4 bg-purple-50 rounded-xl border border-purple-100">
            <div>
                ...
                <input type="text" name="topic_id" value="{{ old('topic_id') }}"
            ...
            <div>
                ...
                <input type="number" name="difficulty_weight" id="difficulty_weight"
                       value="{{ old('difficulty_weight', '1.00') }}"
                       min="0.1" max="5" step="0.1"
                       class="w-full border-gray-300 rounded-lg shadow-sm focus:border-purple-400 focus:ring-purple-400 text-sm">
                <p class="text-xs text-gray-400 mt-1">
                    Auto-filled: Easy=0.5, Medium=1.0, Hard=2.0. Override for special questions.
                </p>
            </div>
        </div>
        ...
        <div class="p-4 bg-gray-50 rounded-xl border border-gray-200 space-y-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                    ...
                    <input type="text" name="option_0" required placeholder="Warning / Danger Sign"
                ...
                <div>
                    ...
                    <input type="text" name="option_1" required placeholder="Mandatory Direction Sign"
                           class="w-full border-gray-300 rounded-lg shadow-sm text-xs">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1">Option C (Index 2)</label>
                    <input type="text" name="option_2" required placeholder="Informational Sign"
                           class="w-full border-gray-300 rounded-lg shadow-sm text-xs">
                </div>
                <div>
                    <label class="block text-xs font-semibold text-gray-600 mb-1">Option D (Index 3)</label>
                    <input type="text" name="option_3" required placeholder="Speed Limit Sign"
                           class="w-full border-gray-300 rounded-lg shadow-sm text-xs">
                </div>
            </div>
            ...
            <div>
                ...
                <select name="correct_index" required class="w-full border-blue-300 bg-blue-50 text-blue-900 rounded-lg shadow-sm text-sm font-semibold">
                    ...
                    <option value="3">Option D (Index 3) is Correct</option>
                </select>
            </div>
        </div>

        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Question Image / Traffic Diagram (Optional)</label>
        ...
        <div>
            ...
            <textarea name="explanation" rows="2" placeholder="e.g. Triangular signs indicate warning or hazards ahead."
                      class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm"></textarea>
        </div>
...
function autoWeight(difficulty) {
    ...
    if (w) w.value = map[difficulty] ?? '1.00';
}
// Init on load
autoWeight(document.getElementById('difficulty_select')?.value ?? 'medium');
</script>
@endsection
