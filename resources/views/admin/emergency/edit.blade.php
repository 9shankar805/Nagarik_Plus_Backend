...
<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-8 max-w-3xl">
    <form method="POST" action="{{ route('admin.emergency.update', $emergency) }}" class="space-y-6">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="name" value="{{ old('name', $emergency->name) }}" required
            ...
            <div>
                ...
                <input type="text" name="name_np" value="{{ old('name_np', $emergency->name_np) }}"
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="number" value="{{ old('number', $emergency->number) }}" required
            ...
            <div>
                ...
                <select name="category" required
                    ...
                    @foreach(['police', 'ambulance', 'fire', 'disaster', 'health'] as $cat)
                        <option value="{{ $cat }}" {{ old('category', $emergency->category) === $cat ? 'selected' : '' }}>{{ ucfirst($cat) }}</option>
                    @endforeach
                </select>
        ...
        <div>
            ...
            <textarea name="description" rows="3"
                      class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('description', $emergency->description) }}</textarea>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Icon</label>
                <input type="text" name="icon" value="{{ old('icon', $emergency->icon) }}"
                       class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1.5">Color</label>
                <input type="text" name="color" value="{{ old('color', $emergency->color) }}"
            ...
            <div>
                ...
                <input type="number" name="sort_order" value="{{ old('sort_order', $emergency->sort_order) }}" min="0"
                       class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
        </div>
        ...
        <div>
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_active" value="1" {{ old('is_active', $emergency->is_active) ? 'checked' : '' }}
