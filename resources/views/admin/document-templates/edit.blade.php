...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-900 mt-2">Edit Template — <span class="text-gray-500 font-normal text-xl">{{ $template->title }}</span></h1>
    <p class="text-sm text-gray-600">Last updated {{ $template->updated_at?->diffForHumans() ?? 'just created' }} · {{ $template->download_count }} downloads</p>
</div>

<form action="{{ route('admin.document-templates.update', $template) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    @csrf
    @method('PUT')
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        @include('admin.document-templates.partials.form', ['tpl' => $template])
