    ...
    <form method="POST" action="{{ route('admin.services.update', $service) }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="url" name="video_url" value="{{ old('video_url', $service->video_url) }}" placeholder="https://…"
                       class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">
            </div>
        </div>
        ...
        <div>
            ...
            @if($service->video_url)
                <p class="mt-1 text-xs text-gray-500">Current: {{ basename($service->video_url) }}</p>
            @endif
