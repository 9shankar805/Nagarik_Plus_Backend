...
@section('subtitle', 'Create a new premium access plan')
...
<div class="mb-6">
    ...
    <h2 class="text-2xl font-bold text-gray-900 mt-2">Add Subscription Package</h2>
</div>

<form action="{{ route('admin.learning.subscriptions.store') }}" method="POST">
    @csrf
    <div class="grid grid-cols-3 gap-6">
        ...
        <div class="col-span-2 space-y-5">
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="price" value="{{ old('price') }}" required min="0" step="0.01"
                               placeholder="e.g. 299"
                               class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#4A5D4A] @error('price') border-red-400 @enderror">
                ...
                <div>
                    ...
                    <div class="flex gap-3 flex-wrap">
                        @foreach([7 => '7 Days', 30 => '1 Month', 90 => '3 Months', 180 => '6 Months', 365 => '1 Year'] as $days => $label)
                        <label class="flex items-center gap-1.5 cursor-pointer">
                            <input type="radio" name="duration_days" value="{{ $days }}"
                                   {{ old('duration_days') == $days ? 'checked' : '' }}
                                   class="text-[#4A5D4A]">
                    ...
                    <input type="number" name="duration_days" value="{{ old('duration_days') }}" min="1"
                           ...
                           class="mt-2 w-48 border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#4A5D4A] @error('duration_days') border-red-400 @enderror">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
                ...
                <p class="text-xs text-gray-500">List what this package includes. These display on the subscription page.</p>
                ...
                <div id="features-list" class="space-y-2">
                    ...
                    <div class="flex gap-2 items-center feature-row">
                        <input type="text" name="features[]" value="{{ $feat }}"
                               placeholder="e.g. Unlimited Mock Tests"
