...
<div class="mb-6">
    ...
    <h2 class="text-2xl font-bold text-gray-900 mt-2">Edit: {{ $achievement->icon }} {{ $achievement->title_en }}</h2>
</div>

<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 max-w-2xl space-y-5">
    <form action="{{ route('admin.learning.achievements.update', $achievement) }}" method="POST">
        @csrf @method('PUT')
        <div class="grid grid-cols-2 gap-4">
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Slug (read-only)</label>
                <input type="text" value="{{ $achievement->slug }}" readonly
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="title_en" value="{{ old('title_en', $achievement->title_en) }}" required
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#4A5D4A]">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Title (NP)</label>
                <input type="text" name="title_np" value="{{ old('title_np', $achievement->title_np) }}"
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#4A5D4A]">
            </div>
        </div>
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <textarea name="description_en" rows="2"
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#4A5D4A]">{{ old('description_en', $achievement->description_en) }}</textarea>
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Description (NP)</label>
                <textarea name="description_np" rows="2"
                          class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#4A5D4A]">{{ old('description_np', $achievement->description_np) }}</textarea>
            </div>
        </div>
        <div class="grid grid-cols-3 gap-4">
            ...
            <div>
                ...
                <input type="number" name="threshold" value="{{ old('threshold', $achievement->threshold) }}" min="1" required
                       class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-[#4A5D4A]">
            </div>
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1">Badge Color</label>
                <input type="color" name="badge_color" value="{{ old('badge_color', $achievement->badge_color) }}"
        ...
        <div class="flex gap-3 pt-2">
            ...
            <a href="{{ route('admin.learning.achievements.index') }}" class="px-6 py-2.5 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 font-medium text-sm">Cancel</a>
        </div>
    </form>
</div>
@endsection
