...
@section('title', 'Create Road Sign')
    ...
    <form method="POST" action="{{ route('admin.road-signs.store') }}" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <textarea name="meaning" rows="3" required
                          class="w-full px-4 py-2.5 border rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 {{ $errors->has('meaning') ? 'border-red-400' : 'border-gray-300' }}">{{ old('meaning') }}</textarea>
            ...
            <div>
                ...
                <textarea name="meaning_np" rows="3"
                          class="w-full px-4 py-2.5 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500">{{ old('meaning_np') }}</textarea>
            </div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="text" name="color_code" value="{{ old('color_code') }}" placeholder="#FF0000"
