...
<div class="mb-6">
    ...
    <h1 class="text-2xl font-bold text-gray-900 mt-2">Edit Educational Video Short #{{ $short->id }}</h1>
</div>

<div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6 max-w-2xl">
    <form action="{{ route('admin.shorts.update', $short) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
        @csrf
        @method('PUT')

        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Title (English)</label>
            <input type="text" name="title_en" value="{{ $short->title_en }}" required
        ...
        <div>
            ...
            <input type="text" name="title_np" value="{{ $short->title_np }}"
                   class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
        </div>
        ...
        <div>
            ...
            <select name="category" required class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">
                <option value="traffic_rules" @if($short->category === 'traffic_rules') selected @endif>Traffic Rules & Signs</option>
                <option value="license_prep" @if($short->category === 'license_prep') selected @endif>Driving License Preparation</option>
                <option value="citizen_guides" @if($short->category === 'citizen_guides') selected @endif>Citizen Services Guide</option>
                <option value="passport_guides" @if($short->category === 'passport_guides') selected @endif>Passport & Visa Guide</option>
            </select>
        </div>
        ...
        <div class="p-4 bg-gray-50 rounded-xl border border-gray-200 space-y-3">
            ...
            <div>
                <label class="block text-xs font-semibold text-gray-600 mb-1">Replace MP4 Video File</label>
            ...
            <div>
                ...
                <input type="url" name="video_url_text" value="{{ $short->video_url }}"
                       class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 text-xs">
            </div>
        </div>

        <div>
            <label class="block text-sm font-semibold text-gray-700 mb-1">Replace Thumbnail Cover Image</label>
        ...
        <div>
            ...
            <textarea name="description_en" rows="3"
                      class="w-full border-gray-300 rounded-lg shadow-sm focus:border-blue-500 focus:ring-blue-500 text-sm">{{ $short->description_en }}</textarea>
        </div>

        <div>
            <label class="flex items-center gap-2 cursor-pointer">
                <input type="checkbox" name="is_published" value="1" @if($short->is_published) checked @endif class="w-4 h-4 text-blue-600 rounded">
